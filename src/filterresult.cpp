...
FilterResult::~FilterResult() { delete mCorrectionMatrix; }
...
void FilterResult::addMergedPairs(int pairs) { mMergedPairs += pairs; }
...
void FilterResult::incCorrectedReads(int count) { mCorrectedReads += count; }
...
void FilterResult::reportAdapterJson(ofstream &ofs, string padding) {
  ...
  ofs << padding << "\t"
      << "\"adapter_trimmed_reads\": " << mTrimmedAdapterRead << "," << endl;
  ofs << padding << "\t"
...
/*void FilterResult::reportHtml(ofstream& ofs, long totalReads) {
    ...
    string labels[4] = {"good_reads", "low_quality_reads", "too_many_N_reads",
"too_short_reads"}; long counts[4] = {mFilterReadStats[PASS_FILTER],
mFilterReadStats[FAIL_QUALITY], mFilterReadStats[FAIL_N_BASE],
mFilterReadStats[FAIL_LENGTH]};

    string json_str = "var data=[";
    json_str += "{values:[" + Stats::list2string(counts, types) + "],";
    json_str += "labels:['good_reads', 'low_quality_reads', 'too_many_N_reads',
'too_short_reads'],"; json_str += "textinfo: 'none',"; json_str +=
"type:'pie'}];\n"; string title = "Filtering statistics of sampled " +
to_string(totalReads) + " reads"; json_str += "var layout={title:'" + title +
"', width:800, height:400};\n"; json_str += "Plotly.newPlot('" + divName + "',
data, layout);\n";
...
void FilterResult::reportAdapterHtml(ofstream &ofs, long totalBases) {
  ofs << "<div class='subsection_title' "
         "onclick=showOrHide('read1_adapters')>Adapter or bad ligation of "
         "read1</div>\n";
  ...
  if (mOptions->isPaired()) {
    ofs << "<div class='subsection_title' "
           "onclick=showOrHide('read2_adapters')>Adapter or bad ligation of "
           "read2</div>\n";
...
void FilterResult::outputAdaptersHtml(
  ...
  ofs << "<table class='summary_table'>\n";
  ofs << "<tr><td class='adapter_col' "
         "style='font-size:14px;color:#ffffff;background:#556699'>"
      << "Sequence"
      << "</td><td class='col2' "
         "style='font-size:14px;color:#ffffff;background:#556699'>"
      << "Occurrences"
