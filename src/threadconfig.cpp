...
ThreadConfig::~ThreadConfig() { cleanup(); }
...
// if a task of writting N files is assigned to this thread, but the input file
// doesn't have so many reads to input write some empty files so it will not
// break following pipelines
...
bool ThreadConfig::canBeStopped() { return mCanBeStopped; }