...
int main(int argc, char *argv[]) {
  ...
  cmd.add("merge", 'm',
          ...
          "file given by --merged_out, the unmerged reads will be written to "
          "the files specified by --out1 and --out2. The merging mode is "
          "disabled by default.");
  cmd.add<string>("merged_out", 0,
                  "in the merging mode, specify the file name to store merged "
                  "output, or specify --stdout to stream the merged output",
                  false, "");
  cmd.add("include_unmerged", 0,
  ...
  cmd.add<int>("compression", 'z',
               "compression level for gzip output (1 ~ 9). 1 is fastest, 9 is "
               "smallest, default is 4.",
               false, 4);
  cmd.add("stdin", 0,
  ...
  cmd.add<int>("reads_to_process", 0,
  ...
  // adapter
  cmd.add("disable_adapter_trimming", 'A',
          "adapter trimming is enabled by default. If this option is "
          "specified, adapter trimming is disabled");
  cmd.add<string>("adapter_sequence", 'a',
                  "the adapter for read1. For SE data, if not specified, the "
                  "adapter will be auto-detected. For PE data, this is used if "
                  "R1/R2 are found not overlapped.",
                  false, "auto");
  cmd.add<string>("adapter_sequence_r2", 0,
                  "the adapter for read2 (PE data only). This is used if R1/R2 "
                  "are found not overlapped. If not specified, it will be the "
                  "same as <adapter_sequence>",
                  false, "auto");
  cmd.add<string>("adapter_fasta", 0,
                  "specify a FASTA file to trim both read1 and read2 (if PE) "
                  "by all the sequences in this FASTA file",
                  false, "");
  cmd.add("detect_adapter_for_pe", 0,
          "by default, the auto-detection for adapter is for SE data input "
  ...
  // cutting by quality
  cmd.add("cut_front", '5',
          "move a sliding window from front (5') to tail, drop the bases in "
          "the window if its mean quality < threshold, stop otherwise.");
  cmd.add("cut_tail", '3',
          "move a sliding window from tail (3') to front, drop the bases in "
          "the window if its mean quality < threshold, stop otherwise.");
  cmd.add("cut_right", 'r',
          "move a sliding window from front to tail, if meet one window with "
          "mean quality < threshold, drop the bases in the window and the "
          "right part, and then stop.");
  cmd.add<int>("cut_window_size", 'W',
               "the window size option shared by cut_front, cut_tail or "
               "cut_sliding. Range: 1~1000, default: 4",
               false, 4);
  cmd.add<int>("cut_mean_quality", 'M',
               "the mean quality requirement option shared by cut_front, "
               "cut_tail or cut_sliding. Range: 1~36 default: 20 (Q20)",
               false, 20);
  cmd.add<int>("cut_front_window_size", 0,
               "the window size option of cut_front, default to "
               "cut_window_size if not specified",
               false, 4);
  cmd.add<int>("cut_front_mean_quality", 0,
               "the mean quality requirement option for cut_front, default to "
               "cut_mean_quality if not specified",
               false, 20);
  cmd.add<int>("cut_tail_window_size", 0,
               "the window size option of cut_tail, default to cut_window_size "
               "if not specified",
               false, 4);
  cmd.add<int>("cut_tail_mean_quality", 0,
               "the mean quality requirement option for cut_tail, default to "
               "cut_mean_quality if not specified",
               false, 20);
  cmd.add<int>("cut_right_window_size", 0,
               "the window size option of cut_right, default to "
               "cut_window_size if not specified",
               false, 4);
  cmd.add<int>("cut_right_mean_quality", 0,
               "the mean quality requirement option for cut_right, default to "
               "cut_mean_quality if not specified",
               false, 20);

  // quality filtering
  cmd.add("disable_quality_filtering", 'Q',
          "quality filtering is enabled by default. If this option is "
          "specified, quality filtering is disabled");
  cmd.add<int>("qualified_quality_phred", 'q',
               "the quality value that a base is qualified. Default 15 means "
               "phred quality >=Q15 is qualified.",
               false, 15);
  cmd.add<int>("unqualified_percent_limit", 'u',
               "how many percents of bases are allowed to be unqualified "
               "(0~100). Default 40 means 40%",
               false, 40);
  cmd.add<int>("n_base_limit", 'n',
               "if one read's number of N base is >n_base_limit, then this "
               "read/pair is discarded. Default is 5",
               false, 5);
  cmd.add<int>("average_qual", 'e',
  ...
  cmd.add("low_complexity_filter", 'y',
          "enable low complexity filter. The complexity is defined as the "
          "percentage of base that is different from its next base (base[i] != "
          "base[i+1]).");
  cmd.add<int>("complexity_threshold", 'Y',
               "the threshold for low complexity filter (0~100). Default is "
               "30, which means 30% complexity is required.",
               false, 30);

  // filter by indexes
  cmd.add<string>("filter_by_index1", 0,
                  "specify a file contains a list of barcodes of index1 to be "
                  "filtered out, one barcode per line",
                  false, "");
  cmd.add<string>("filter_by_index2", 0,
                  "specify a file contains a list of barcodes of index2 to be "
                  "filtered out, one barcode per line",
                  false, "");
  cmd.add<int>("filter_by_index_threshold", 0,
               "the allowed difference of index barcode for index filtering, "
               "default 0 means completely identical.",
               false, 0);

  // base correction in overlapped regions of paired end data
  cmd.add("correction", 'c',
          "enable base correction in overlapped regions (only for PE data), "
          "default is disabled");
  cmd.add<int>("overlap_len_require", 0,
               "the minimum length to detect overlapped region of PE reads. "
               "This will affect overlap analysis based PE merge, adapter "
               "trimming and correction. 30 by default.",
               false, 30);
  cmd.add<int>("overlap_diff_limit", 0,
               "the maximum number of mismatched bases to detect overlapped "
  ...
  if (cmd.exist("discard_unmerged")) {
    cerr << "DEPRECATED: --discard_unmerged has no effect now, see the "
  ...
  if (!opt.qualityCut.enabledFront) {
    ...
    if (opt.qualityCut.enabledFront)
      cerr << "WARNING: cut_by_quality5 is deprecated, please use cut_front "
              "instead."
           << endl;
  }
  ...
  if (!opt.qualityCut.enabledFront) {
    ...
    if (opt.qualityCut.enabledFront)
      cerr << "WARNING: cut_by_quality3 is deprecated, please use cut_tail "
              "instead."
           << endl;
  }
  ...
  if (!opt.qualityCut.enabledRight) {
    ...
    if (opt.qualityCut.enabledRight)
      cerr << "WARNING: cut_by_quality_aggressive is deprecated, please use "
              "cut_right instead."
           << endl;
  }
  ...
  if (opt.split.needEvaluation && supportEvaluation) {
    ...
    if (opt.split.size <= 0) {
      opt.split.size = 1;
      cerr << "WARNING: the input file has less reads than the number of files "
              "to split"
           << endl;
    }
  }
