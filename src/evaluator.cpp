...
Evaluator::Evaluator(Options *opt) { mOptions = opt; }
...
void Evaluator::evaluateReadNum(long &readNum) {
  ...
  if (reachedEOF) {
    ...
    // by the way, update readNum so we don't need to evaluate it if splitting
    // output is enabled
    reader.getBytes(bytesRead, bytesTotal);
    double bytesPerRead = (double)(bytesRead - firstReadPos) / (double)records;
    // increase it by 1% since the evaluation is usually a bit lower due to bad
    // quality causes lower compression rate
...
string Evaluator::evalAdapterAndReadNumDepreciated(long &readNum) {
  ...
  // why we add trim_tail here? since the last cycle are usually with low
  // quality and should be trimmed
  const int keylen = 10;
  int size = 1 << (keylen * 2);
