...
bool Options::isPaired() { return in2.length() > 0 || interleavedInput; }
...
bool Options::polyXTrimmingEnabled() { return polyXTrim.enabled; }
...
bool Options::validate() {
  if (in1.empty()) {
    if (!in2.empty())
      error_exit("read2 input is specified by <in2>, but read1 input is not "
                 "specified by <in1>");
    if (inputFromSTDIN)
      in1 = "/dev/stdin";
    else
      error_exit("read1 input should be specified by --in1, or enable --stdin "
                 "if you want to read STDIN");
  } else {
  ...
  if (merge.enabled) {
    ...
    if (merge.out.empty() && !outputToSTDOUT) {
      error_exit("In merging mode, you should either specify --merged_out or "
                 "enable --stdout");
    }
    if (!merge.out.empty()) {
      if (merge.out == out1)
  ...
  if (in2.empty() && !interleavedInput && !out2.empty()) {
    error_exit("read2 output is specified (--out2), but neighter read2 input "
               "is not specified (--in2), nor read1 is interleaved.");
  }

  if (!in2.empty() || interleavedInput) {
    if (!out1.empty() && out2.empty()) {
      error_exit("paired-end input, read1 output should be specified together "
                 "with read2 output (--out2 needed) ");
    }
    if (out1.empty() && !out2.empty()) {
      if (!merge.enabled)
        error_exit("paired-end input, read1 output should be specified (--out1 "
                   "needed) together with read2 output ");
    }
  }

  if (!in2.empty() && interleavedInput) {
    error_exit("<in2> is not allowed when <in1> is specified as interleaved "
               "mode by (--interleaved_in)");
  }

  if (!out1.empty()) {
    // check_file_writable(out1);
    if (out1 == out2) {
      error_exit("read1 output (--out1) and read1 output (--out2) should be "
                 "different");
    }
    if (dontOverwrite && file_exists(out1)) {
      error_exit(out1 + " already exists and you have set to not rewrite "
                        "output files by --dont_overwrite");
    }
  }
  if (!out2.empty()) {
    // check_file_writable(out2);
    if (dontOverwrite && file_exists(out2)) {
      error_exit(out2 + " already exists and you have set to not rewrite "
                        "output files by --dont_overwrite");
    }
  }
  if (!overlappedOut.empty()) {
    ...
    if (dontOverwrite && file_exists(overlappedOut)) {
      error_exit(overlappedOut + " already exists and you have set to not "
                                 "rewrite output files by --dont_overwrite");
    }
  }
  if (!isPaired()) {
    if (!unpaired1.empty()) {
  ...
  if (!unpaired1.empty()) {
    if (dontOverwrite && file_exists(unpaired1)) {
      error_exit(unpaired1 + " already exists and you have set to not rewrite "
                             "output files by --dont_overwrite");
    }
    if (unpaired1 == out1)
  ...
  if (!unpaired2.empty()) {
    if (dontOverwrite && file_exists(unpaired2)) {
      error_exit(unpaired2 + " already exists and you have set to not rewrite "
                             "output files by --dont_overwrite");
    }
    if (unpaired2 == out1)
  ...
  if (!failedOut.empty()) {
    if (dontOverwrite && file_exists(failedOut)) {
      error_exit(failedOut + " already exists and you have set to not rewrite "
                             "output files by --dont_overwrite");
    }
    if (failedOut == out1)
  ...
  if (dontOverwrite) {
    if (file_exists(jsonFile)) {
      error_exit(jsonFile + " already exists and you have set to not rewrite "
                            "output files by --dont_overwrite");
    }
    if (file_exists(htmlFile)) {
      error_exit(htmlFile + " already exists and you have set to not rewrite "
                            "output files by --dont_overwrite");
    }
  }

  if (compression < 1 || compression > 9)
    error_exit("compression level (--compression) should be between 1 ~ 9, 1 "
               "for fastest, 9 for smallest");

  if (readsToProcess < 0)
    error_exit("the number of reads to process (--reads_to_process) cannot be "
               "negative");
  ...
  if (qualfilter.qualifiedQual - 33 < 0 || qualfilter.qualifiedQual - 33 > 93)
    error_exit("qualitified phred (--qualified_quality_phred) should be 0 ~ "
               "93, suggest 10 ~ 20");
  ...
  if (overlapDiffPercentLimit < 0 || overlapDiffPercentLimit > 100)
    error_exit("the maximum percentage of mismatched bases to detect "
               "overlapped region (--overlap_diff_percent_limit) should be 0 ~ "
               "100, suggest 20 ~ 60");

  if (split.enabled) {
    if (split.digits < 0 || split.digits > 10)
      error_exit("you have enabled splitting output to multiple files, the "
                 "digits number of file name prefix (--split_prefix_digits) "
                 "should be 0 ~ 10.");

    if (split.byFileNumber) {
      if (split.number < 2 || split.number >= 1000)
        error_exit("you have enabled splitting output by file number, the "
                   "number of files (--split) should be 2 ~ 999.");
    ...
    if (split.byFileLines) {
      if (split.size < 1000 / 4)
        error_exit("you have enabled splitting output by file lines, the file "
                   "lines (--split_by_lines) should be >= 1000.");
    }
  }
  ...
  if (qualityCut.enabledFront || qualityCut.enabledTail ||
    ...
    if (qualityCut.qualityShared < 1 || qualityCut.qualityShared > 30)
      error_exit("the mean quality requirement for cutting by quality "
                 "(--cut_mean_quality) should be 1 ~ 30, suggest 15 ~ 20.");
  ...
  if (adapter.sequence != "auto" && !adapter.sequence.empty()) {
    ...
    for (int i = 0; i < adapter.sequence.length(); i++) {
      ...
      if (c != 'A' && c != 'T' && c != 'C' && c != 'G') {
        error_exit("the adapter <adapter_sequence> can only have bases in {A, "
                   "T, C, G}, but the given sequence is: " +
                   adapter.sequence);
      }
    }
  ...
  if (adapter.sequenceR2 != "auto" && !adapter.sequenceR2.empty()) {
    ...
    for (int i = 0; i < adapter.sequenceR2.length(); i++) {
      ...
      if (c != 'A' && c != 'T' && c != 'C' && c != 'G') {
        error_exit("the adapter <adapter_sequence_r2> can only have bases in "
                   "{A, T, C, G}, but the given sequenceR2 is: " +
                   adapter.sequenceR2);
      }
    }
  ...
  if (umi.enabled) {
    if (umi.location == UMI_LOC_READ1 || umi.location == UMI_LOC_READ2 ||
      ...
      if (umi.skip > 0)
        error_exit("Only if the UMI location is in read1/read2/per_read, you "
                   "can skip bases after UMI");
      if (umi.length > 0)
        error_exit("Only if the UMI location is in read1/read2/per_read, you "
                   "can set the UMI length");
    }
    if (!umi.prefix.empty()) {
    ...
    if (!umi.separator.empty()) {
      ...
      for (int i = 0; i < umi.separator.length(); i++) {
        ...
        if (c != 'A' && c != 'T' && c != 'C' && c != 'G') {
          error_exit("UMI separator can only have bases in {A, T, C, G}, but "
