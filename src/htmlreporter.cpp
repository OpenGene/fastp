...
void HtmlReporter::setDup(double dupRate) { mDupRate = dupRate; }
...
void HtmlReporter::printSummary(ofstream &ofs, FilterResult *result,
  ...
  ofs << endl;
  ofs << "<h1 style='text-align:left;'><a "
         "href='https://github.com/OpenGene/fastp' target='_blank' "
         "style='color:#663355;text-decoration:none;'>" +
             mOptions->reportTitle + "</a>"
      << endl;
  ofs << "<div class='section_div'>\n";
  ofs << "<div class='section_title' onclick=showOrHide('summary')><a "
         "name='summary'>Summary</a></div>\n";
  ofs << "<div id='summary'>\n";

  ofs << "<div class='subsection_title' "
         "onclick=showOrHide('general')>General</div>\n";
  ofs << "<div id='general'>\n";
  ...
  ofs << "<div class='subsection_title' "
         "onclick=showOrHide('before_filtering_summary')>Before "
         "filtering</div>\n";
  ...
  ofs << "<div class='subsection_title' "
         "onclick=showOrHide('after_filtering_summary')>After "
         "filtering</div>\n";
  ...
  if (result) {
    ofs << "<div class='subsection_title' "
           "onclick=showOrHide('filtering_result')>Filtering result</div>\n";
  ...
  if (result && mOptions->adapterCuttingEnabled()) {
    ofs << "<div class='section_div'>\n";
    ofs << "<div class='section_title' onclick=showOrHide('adapters')><a "
           "name='summary'>Adapters</a></div>\n";
    ofs << "<div id='adapters'>\n";
  ...
  /*if(mOptions->duplicate.enabled) {
      ofs << "<div class='section_div'>\n";
      ofs << "<div class='section_title' onclick=showOrHide('duplication')><a
  name='summary'>Duplication</a></div>\n"; ofs << "<div id='duplication'>\n";
  ...
  if (mOptions->isPaired()) {
    ofs << "<div class='section_div'>\n";
    ofs << "<div class='section_title' onclick=showOrHide('insert_size')><a "
...
void HtmlReporter::reportInsertSize(ofstream &ofs, int isizeLimit) {
  ...
  ofs << "<div class='sub_section_tips'>This estimation is based on paired-end "
         "overlap analysis, and there are ";
  ofs << to_string(unknownPercents);
  ofs << "% reads found not overlapped. <br /> The nonoverlapped read pairs "
         "may have insert size &lt;"
      << mOptions->overlapRequire;
  ofs << " or &gt;" << isizeLimit;
  ofs << ", or contain too much sequencing errors to be detected as "
         "overlapped.";
  ofs << "</div>\n";
...
void HtmlReporter::reportDuplication(ofstream &ofs) {
  ...
  ofs << "<div class='figure' id='plot_duplication' "
         "style='height:400px;'></div>\n";
  ofs << "</div>\n";
...
void HtmlReporter::report(FilterResult *result, Stats *preStats1,
  ...
  ofs << "<div class='section_div'>\n";
  ofs << "<div class='section_title' onclick=showOrHide('before_filtering')><a "
         "name='summary'>Before filtering</a></div>\n";
  ...
  ofs << "<div class='section_div'>\n";
  ofs << "<div class='section_title' onclick=showOrHide('after_filtering')><a "
         "name='summary'>After filtering</a></div>\n";
...
void HtmlReporter::printHeader(ofstream &ofs) {
  ofs << "<html><head><meta http-equiv=\"content-type\" "
...
void HtmlReporter::printCSS(ofstream &ofs) {
  ...
  ofs << "table {border:1px solid #999999;padding:2x;border-collapse:collapse; "
         "width:800px}"
      << endl;
  ofs << ".col1 {width:240px; font-weight:bold;}" << endl;
  ofs << ".adapter_col {width:500px; font-size:10px;}" << endl;
  ofs << "img {padding:30px;}" << endl;
  ofs << "#menu {font-family:Consolas, 'Liberation Mono', Menlo, Courier, "
         "monospace;}"
      << endl;
  ofs << "#menu a {color:#0366d6; "
         "font-size:18px;font-weight:600;line-height:28px;text-decoration:none;"
  ...
  ofs << ".section_title "
      ...
      << endl;
  ofs << ".subsection_title "
         "{font-size:16px;padding:5px;margin-top:10px;text-align:left;color:#"
         "663355}"
      << endl;
  ofs << "#container {text-align:center;padding:3px 3px 3px "
         "10px;font-family:Arail,'Liberation Mono', Menlo, Courier, monospace;}"
      << endl;
  ofs << ".menu_item {text-align:left;padding-top:5px;font-size:18px;}" << endl;
  ofs << ".highlight "
         "{text-align:left;padding-top:30px;padding-bottom:30px;font-size:20px;"
         "line-height:35px;}"
      << endl;
  ofs << "#helper {text-align:left;border:1px dotted "
         "#fafafa;color:#777777;font-size:12px;}"
      << endl;
  ofs << "#footer "
         "{text-align:left;padding:15px;color:#ffffff;font-size:10px;"
         "background:#663355;font-family:Arail,'Liberation Mono', Menlo, "
         "Courier, monospace;}"
      << endl;
  ofs << ".kmer_table {text-align:center;font-size:8px;padding:2px;}" << endl;
  ofs << ".kmer_table "
         "td{text-align:center;font-size:8px;padding:0px;color:#ffffff}"
      << endl;
  ofs << ".sub_section_tips "
         "{color:#999999;font-size:10px;padding-left:5px;padding-bottom:3px;}"
      << endl;
  ofs << "</style>" << endl;
}
